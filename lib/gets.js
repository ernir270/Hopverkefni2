    ...
    for(let i = 0; i < dataItems.length; i++){
        for(let j = 0; j < dataItems[i].tags.length; j++){
            ...
            for(let g = 0; g < tagArr.length; g++){
                // ef tagArr[g] er með dataItems[i].tags[j] í listanum  && !dataItems[i].deleted
                if(tagArr[g].includes(dataItems[i].tags[j]) && !dataItems[i].deleted){
                    containsIt = true;
            ...
            if(!containsIt && !dataItems[i].deleted){
...
export function showTaskCount(){
    ...
    // ef að það er núþegar búið að búa til <p>, þá breytum við bara textanum
    if(document.querySelector('.taskbuttons-container').querySelectorAll(".right-num").length > 0){
