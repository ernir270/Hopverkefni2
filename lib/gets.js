        ...
        for(let j = 0; j < dataItems[i].tags.length; j++){
            ...
            for(let g = 0; g < tagArr.length; g++){
                ...
                if(tagArr[g].includes(dataItems[i].tags[j]) &&
                   !dataItems[i].deleted && !dataItems[i].completed){
                    containsIt = true;
 ...
 */
 export function getTags(){
    ...
    const tagArr = getTagArray();
    
    const tagCont = document.querySelector('.tag-container');

    // eyðir öllum tags ef þau eru núþegar birtuð
    const TagList = tagCont.querySelectorAll('.li-container');
    for(let i = 0; i < TagList.length; i++){
        tagCont.removeChild(TagList[i]);
    }


    for(let i = 0; i < tagArr.length; i++){ // búum til element og birtum tag-in

        const liCont = document.createElement('div'); // búa til div
...
export function getCategories(){
    ...
    const categoryArray = getCaregoryArray();
    ...
    for(let i = 0; i < categoryArray.length; i++){
        ...
        const li = document.createElement('li');
        const categoryTitle = categoryArray[i][0];
